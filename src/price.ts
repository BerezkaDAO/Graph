...
export function handleBlock(block: ethereum.Block): void {
    ...
    // Speed up parsing by throwing away 29 blocks out of 30 
    ...
    let flexPrice    = priceTracker.getPrice(Address.fromString(FLEX_TOKEN))
    let eflxPrice    = priceTracker.getPrice(Address.fromString(EFLX_TOKEN))
    let bdqPrice     = priceTracker.getPrice(Address.fromString(BDQ_TOKEN))
    let dynaPrice    = priceTracker.getPrice(Address.fromString(DYNA_TOKEN))
    let skyflexPrice = priceTracker.getPrice(Address.fromString(SKYFLEX_TOKEN))
    ...
    handleDay(timestamp, flexPrice, flexSupply, FLEX_TOKEN, DAYID_INIT_FLEX)
    handleDay(timestamp, eflxPrice, eflxSupply, EFLX_TOKEN, DAYID_INIT_EFLX)
    handleDay(timestamp, bdqPrice , bdqSupply , BDQ_TOKEN , DAYID_INIT_BDQ)
    handleDay(timestamp, dynaPrice, dynaSupply, DYNA_TOKEN, DAYID_INIT_DYNA)
    handleDay(timestamp, skyflexPrice, skyflexSupply, SKYFLEX_TOKEN, DAYID_INIT_SKYFLEX)

    handleHour(timestamp, flexPrice, flexSupply, FLEX_TOKEN)
    handleHour(timestamp, eflxPrice, eflxSupply, EFLX_TOKEN)
    handleHour(timestamp, bdqPrice , bdqSupply , BDQ_TOKEN)
    handleHour(timestamp, dynaPrice, dynaSupply, DYNA_TOKEN)
